...
function run(settings){
	...
	$bar.progressbar({
		...
		max: (function(){
			var imgcount;
			settings.timeline.some(function(block){
				if((block.type == "similarity" || block.type =="abx") && !block.is_practice){
					imgcount =  (block.length + settings.practices)/2;
					return true;
				}
			})
			return imgcount;
	...
	function incrProgressBar(){
		var old = $bar.progressbar("value");
		$bar.progressbar("value", old+1);
	}
	...
	launcher.loadMicroComponents(settings, function(){
		...
		var exp;
		launcher.createStandardExperiment({
			settings: settings,
			distTweak : distTweak,
			atEach	: incrProgressBar,
